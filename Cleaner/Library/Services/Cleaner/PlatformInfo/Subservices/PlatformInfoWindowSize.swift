//
//  PlatformInfoWindowSize.swift
//  Created at 23.03.2023.
//

import UIKit

public enum PlatformInfoWindowSize {
    
    public enum GetPpiResult {
        case success(ppi: Double)
        case unknown(bestGuessPpi: Double)
    }
    ...
    {
        do {
            let ppi = try lookUp(machineName: PlatformInfoPhoneModel.getDeviceID())
            return .success(ppi: ppi)
        }
        catch {
            let ppi = guess(idiom: UIDevice.current.userInterfaceIdiom, screen: UIScreen.main)
            return .unknown(bestGuessPpi: ppi)
        }
    }
}

extension PlatformInfoWindowSize {
