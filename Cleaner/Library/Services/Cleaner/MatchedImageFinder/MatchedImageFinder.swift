...
/// Если в галерее на устройстве произошли изменения, то сервис перезапускает поиск, а по завершению отправляет   `NotificationRelay.galleryFinderUpdated`
...
class MatchedImageFinder: NSObject {
    
    // MARK: - Singletone
    
    static let shared = MatchedImageFinder()
    ...
    func requestAccess(fullAccessGranted: @escaping VoidCallback, limitedAccessGranted: @escaping VoidCallback, needShowDeniedAlert: @escaping VoidCallback) {
        if #available(iOS 14, *) {
            let status = PHPhotoLibrary.authorizationStatus(for: .readWrite)
    ...
    func getSizeOf(_ type: MatchedImageFinderSectionModel) -> Int64 {
        let data: [PHAsset]
    ...
    ///   - completion: колбэк завершения, может вернуть ошибку
    func smartDeleting(types:  Set<MatchedImageFinderSectionModel>, completion: ((Error?) -> Void)? = nil) {
    ...
    private func checkErrorsForOperations() -> MatchedImageFinderFailure? {
    ...
    private func findMedia() {
        ...
        let operation = BlockOperation { [weak self] in
            ...
            if self.operationQueue.operationCount < 2 {
                ...
                NotificationRelay.send(event: .galleryFinderUpdated)
            }
        }
...
extension MatchedImageFinder: PHPhotoLibraryChangeObserver {
