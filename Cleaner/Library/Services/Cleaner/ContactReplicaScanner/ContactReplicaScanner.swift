...
/// Если контакты на устройстве были обновлены, ты отправляется нотификация  `NotificationRelay.contactFinderUpdated`
...
class ContactReplicaScanner: NSObject {
    
    // MARK: - Singletone
    
    static let shared = ContactReplicaScanner()
    ...
    private var contactVCCallbacks: [CNContactViewController:VoidCallback] = [:]
    ...
    func requestAccess(accessGranted: @escaping VoidCallback, needShowDeniedAlert: @escaping VoidCallback) {
    ...
    func getAll() throws -> [ContactReplicaScannerViewType]  {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        
        let data = allContacts.compactMap { ContactReplicaScannerViewType(cnContact: $0, phoneKit: phoneNumberKit) }
        return data
    }
    
    /// Получить контакты, которые совпадают по имени и номеру
    /// - Returns: Сгруппированные по дубликатам массивы
    func getFullDuplicates() throws -> [[ContactReplicaScannerViewType]]  {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        
        let data = fullDuplicates.map { $0.compactMap { ContactReplicaScannerViewType(cnContact: $0, phoneKit: phoneNumberKit) } }
        return data
    }
    ...
    func getNameDuplicates() throws -> [ContactReplicaScannerViewType: [ContactReplicaScannerViewType]] {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        
        var data: [ContactReplicaScannerViewType:[ContactReplicaScannerViewType]] = [:]
        duplicatesByName.forEach {
            let merged = ContactReplicaScannerViewType(cnContact: $0.key, phoneKit: phoneNumberKit)
            let contacts = $0.value.compactMap { ContactReplicaScannerViewType(cnContact: $0, phoneKit: phoneNumberKit) }
    ...
    /// - Returns: Сгруппированные по дубликатам массивы, ключом массива является объедененный контакт для этой группы
    func getPhoneDuplicates() throws -> [ContactReplicaScannerViewType:[ContactReplicaScannerViewType]]  {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
    ...
    func getWithoutName() throws -> [ContactReplicaScannerViewType]  {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        
        let data = withoutName.compactMap { ContactReplicaScannerViewType(cnContact: $0, phoneKit: phoneNumberKit) }
        return data
    }
    
    /// Получить все контакты без номера телефона
    func getWithoutPhone() throws -> [ContactReplicaScannerViewType]  {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        
        let data = withoutPhone.compactMap { ContactReplicaScannerViewType(cnContact: $0, phoneKit: phoneNumberKit) }
        return data
    }
    ...
    func deleteContacts(_ contacts: [ContactReplicaScannerViewType]) throws {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
    ...
    */
     func smartDeleting(type: ContactReplicaScannerSectionModel) throws {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
    ...
    func getCNContact(for contact: ContactReplicaScannerViewType) -> CNMutableContact? {
    ...
    func saveMergeContactAndDeleteOthers(_ contact: ContactReplicaScannerViewType) throws {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        ...
        guard let dataForOperation = nameDuplicatesData ?? phoneDuplucatesData else {
            throw ContactReplicaScannerFailure.cantFindContact
        }
        
        guard let contactToSave = dataForOperation.key.mutableCopy() as? CNMutableContact else {
            throw ContactReplicaScannerFailure.unexpected(msg: "Failed with create mutable copy for contact")
        }
    ...
    func saveMergeContactsAndDeleteOthers(_ contacts: [ContactReplicaScannerViewType]) throws {
        let error = checkErrorsForOperations()
        if let error = error { throw error }
        let saveRequest = CNSaveRequest()
    ...
    func getNativeContactController(for contact: ContactReplicaScannerViewType, allowEditing: Bool, didCompleteCallback: VoidCallback? = nil) throws -> CNContactViewController  {
        ...
        if let storedContact = storedContact {
            ...
            throw ContactReplicaScannerFailure.unexpected(msg: "Cannot find presented contact")
        }
    ...
    func getSizeOf(_ type: ContactReplicaScannerSectionModel) -> Int64 {
    ...
    private func checkErrorsForOperations() -> ContactReplicaScannerFailure? {
    ...
    private func setupObserers() {
        NotificationRelay.observe(event: .CNContactStoreDidChange) { [weak self] in
    ...
    private func findContacts() {
        ...
        let operation = BlockOperation { [weak self] in
            ...
            if self.operationQueue.operationCount < 2 {
                ...
                NotificationRelay.send(event: .contactFinderUpdated)
            }
        }
...
extension ContactReplicaScanner: CNContactViewControllerDelegate {
